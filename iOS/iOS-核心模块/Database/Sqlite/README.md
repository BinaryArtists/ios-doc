# SQLite

SQLite，是一款轻型的数据库，是遵守ACID的关系型数据库管理系统。它的设计目标是嵌入式的，目前Android和iOS的设备内置的都是SQLite数据库。SQLite虽然娇小，但也支持事务和多数的SQL92标准。

## [常见错误码](http://blog.csdn.net/ameyume/article/details/45504191)

[明细样例表](sqlite_error_code.md)

## 可参考文档

1. [Sqlite.NET官方文档](http://blog.csdn.net/WuLex/article/details/78636047)
2. [Categorical Index Of SQLite Documents](http://tool.oschina.net/apidocs/apidoc?api=sqlite)
3. [sqlite权威指南(第二版）.pdf](http://download.csdn.net/download/tianqibucuo_h/9444272)
4. [SQLite这么娇小可爱，不多了解点都不行啊](http://www.cocoachina.com/ios/20150824/13169.html)
5. [SQLite入门与分析(五)---Page Cache之并发控制](http://www.cnblogs.com/hustcat/archive/2009/03/01/1400757.html)
6. [SQLite入门与分析(六)---再谈SQLite的锁](http://www.cnblogs.com/hustcat/archive/2009/03/10/1408208.html)
7. [iOS端数据库解决方案分析](http://www.cocoachina.com/ios/20161108/18001.html)